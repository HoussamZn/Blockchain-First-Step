...
public:
    MerkleTree(const vector<Transaction>& transactions) {
        ...
        for (auto &tx : transactions)
            tx_hashes.push_back(sha256(tx.to_string()));
    ...
    void build_tree(vector<string>& leaves_hashes) {
        ...
        while (current.size() > 1) {
            ...
            for (size_t i = 0; i < current.size(); i += 2) {
                Node* L = current[i];
                Node* R = current[i + 1];
                Node* parent = new Node(sha256(L->hash + R->hash));
                parent->left = L;
                parent->right = R;
...
struct Block {
    ...
    Block(int idx, const string& prev, const vector<Transaction>& txs)
        : index(idx), prev_hash(prev) 
    {
...
public:
    ...
    Blockchain() {
        ...
        chain.emplace_back(0, "0", genesis_tx);
    }
